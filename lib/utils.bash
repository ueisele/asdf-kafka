...
list_confluent_versions() {
	...
	local versions="$(curl $curl_opts "${CONFLUENT_PACKAGES_URL}/" | sed -rn 's/^[ ]*<a href="\/archive\/.*\/">(.*)\/<\/a>/\1/p' | tr '\n' ' ')"
	# enterprise
	for version in $versions; do
		# extract available versions from html page (without scala version)
		curl $curl_opts ${CONFLUENT_PACKAGES_URL}/${version}/ | sed -rn 's/^.*<a.*>confluent-([0-9\.]*)(-[0-9\.]*)?\.tar\.gz<\/a>/confluent-enterprise-\1/p' | sort -V | uniq
	done
	# community
	for version in $versions; do
		# extract available versions from html page (without scala version)
		curl $curl_opts ${CONFLUENT_PACKAGES_URL}/${version}/ | sed -rn 's/^.*<a.*>confluent-(community|oss)-([0-9\.]*)(-[0-9\.]*)?\.tar\.gz<\/a>/confluent-community-\2/p' | sort -V | uniq
	done
}

list_apache_versions() {
	# extract available versions from html page (without scala version)
	curl $curl_opts "${APACHE_PACKAGES_URL}/" | sed -rn 's/^.*<a href=.*>([0-9\.]*)\/<\/a>.*$/apache-\1/p' | sort -V
}
...
confluent_download_url() {
	...
	# resolve actual version name for download
	...
	# extract most current scala version if present (only until 5.0)
	scalapostfix="$(curl $curl_opts ${CONFLUENT_PACKAGES_URL}/${majorversion}/ | sed -rn "s/^.*${downloadversion}(-[0-9\.]*)?\.tar\.gz<\/a>/\1/p" | sort | tail -1)"
...
apache_download_url() {
	...
	scalaversion="$(curl $curl_opts "${APACHE_PACKAGES_URL}/${downloadversion}/" | sed -rn 's/^.*<a href=.*>kafka_([0-9\.]*)-.*$/\1/p' | uniq | sort -V | tail -1)"
